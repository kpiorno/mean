...
```cpp bind("expr", &Expr::expr, this);``` bind the "expr" method with the "expr" grammar's non-terminal.

The ```cpp float expr(mn::MNNode* node) ...``` method will handle the output generated by the non-terminal "expr".

The ```cpp float walk(mn::MNNode* node) ...``` is the first method which is invoked. The method ```cpp call(node)``` in ```cpp return call(node)``` will invoke the 
